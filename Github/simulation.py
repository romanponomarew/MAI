    ...
    def go_to_service1(self):
        ...
        if self.y <= 280:
            ...
            if self.status == "on_service_station":
                ...
                details_required1 = random.randint(1, 30)
    ...
    def go_to_service2(self):
        ...
        ###############################
            ...
            if self.status == "on_service_station":
                ...
                details_required2 = random.randint(1, 30)
                self.time_station = round(env.now / 1000)
        ...
        while True:
            ...
            if self.status == "in_warehouse" and service_loaders1 == 0 and Loader.to_station1 == 1 \
                ...
                if self.search_status == "search":
                    yield self.env.timeout(self.search_time * details_required1)  # Время поиска запчастей для ст.тех.обсл.1 на складе
            ...
            if self.status == "in_warehouse" and service_loaders2 == 0 and Loader.to_station2 == 1 \
                ...
                if self.search_status == "search":
                    yield self.env.timeout(self.search_time * details_required2)  # Время поиска запчастей для ст.тех.обсл.2 на складе
                    self.search_status = "done"
                if self.search_status == "done":
...
env = PyGameEnvironment(renderer, factor=0.0001, strict=False)   # Для ускоренного воспроизведения модели
...
#######
airplane6 = Airplane(env, 6)
airplane7 = Airplane(env, 7)
airplane8 = Airplane(env, 8)
airplane9 = Airplane(env, 9)
airplane10 = Airplane(env, 10)
##########
