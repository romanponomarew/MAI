...
"""файл для асинхронного моделирования с имзмененными исходными данными(много цветов)"""
"""Другой подсчет штрафа в цикле"""
...
class Agent():
    ...
    def __init__(self, name): #name = цифры
        ...
        if self.name in dict1[agent].neighbors: # Выполняем функцию только если соседи:
            ...
            if self.name not in dict1[agent].current_cycle: #Есть ли значение текущего агента в текущем цикле соседа(i)
                ...
                if len(dict1[agent].neighbors) == len(dict1[agent].current_cycle): #Проверка на законченность цикла соседа
                    ...
                    await dict1[agent].evaluate_cycle(agent)
    ...
    async def func(self, val1, val2, name_neighbor):
    ...
    async def evaluate_cycle(self, name):
        ...
        for key, value in dict1[name].current_cycle.items():
            ...
            cost_current = cost_current + await dict1[name].func(self.val, value, key)
        ...
        for args in dict1[name].domen:
            res1 = 0
            for key, value in dict1[name].current_cycle.items():
                res1 = res1 + await dict1[name].func(args, value, key)
...
#agent1 = random.choice(list(graph["vars"].keys())) # Случайный агент из всех возможных
...
for i in range(0, 1500):
    for agent in graph["vars"].keys():
        tasks.append(event_loop.create_task(dict1[agent].send(random.choice(list(graph["vars"].keys())))))
        c = c +1
        #print("Действие номер - ", c)
print("Длина очереди асинхронных действий=", len(tasks))
...
#########Варианты для обычных тестов##########
# c = 0
# c1 = 10
# for i in range(1500):
#     for agent in graph["vars"].keys():
#         #c1 = c1 - 1
#         dict1[agent].send(random.choice(list(graph["vars"].keys())))
#         c = c +1
#
# results()

#print(choice(list(dict1.keys())))

print("Кол-во действий = ", c)
